...
class TestKeywordSearcher:
    ...
    def test_match_details_tracking(self):
        ...
        index = {
            "modules": {
                "MyApp.UserAccount": {
                    "file": "lib/user_account.ex",
                    "line": 1,
                    "moduledoc": "User account management. Handles user accounts and user data.",
                    "keywords": {"user": 0.9, "account": 0.8, "management": 0.7},
                    "functions": [
                        {
                            "name": "create_user",
                            "arity": 1,
                            "line": 10,
                            "doc": "Creates a new user account. User data is validated.",
                            "keywords": {"create": 0.8, "user": 0.9, "account": 0.7},
                            "string_sources": [
                                {"string": "INSERT INTO users", "line": 15},
                                {"string": "user_account_created", "line": 18},
                            ],
                        }
                    ],
                }
        ...
        assert len(results) == 2  # Module and function both match

        # Check function result has match_details
        func_result = next(r for r in results if r["type"] == "function")
        assert "match_details" in func_result

        match_details = func_result["match_details"]
        assert "user" in match_details

        user_details = match_details["user"]
        assert "total_count" in user_details
        assert "locations" in user_details

        # Should find "user" in:
        # 1. Function name: "create_user" (1 time)
        # 2. Documentation: "Creates a new user account. User data is validated." (2 times)
        # 3. Strings: "user_account_created" (1 time)
        assert user_details["total_count"] >= 3  # At least 3 occurrences

        # Check locations contain the right types
        location_types = [loc["type"] for loc in user_details["locations"]]
    ...
    def test_match_details_string_locations(self):
        ...
        index = {
            "modules": {
                "MyApp.SQL": {
                    "file": "lib/sql.ex",
                    "line": 1,
                    "keywords": {"database": 0.8},
                    "functions": [
                        {
                            "name": "query",
                            "arity": 1,
                            "line": 10,
                            "doc": "Executes SQL query",
                            "keywords": {"query": 0.9},
                            "string_keywords": {"select": 1.04},
                            "string_sources": [
                                {"string": "SELECT * FROM users", "line": 15},
                                {"string": "SELECT id, name FROM accounts", "line": 18},
                                {"string": "SELECT count(*) FROM posts", "line": 22},
                            ],
                        }
                    ],
                }
        ...
        # Should match the function (has string_keywords and string_sources)
        assert len(results) >= 1
        func_result = next(r for r in results if r["type"] == "function")
        assert "match_details" in func_result

        match_details = func_result["match_details"]
        assert "select" in match_details

        select_details = match_details["select"]
        assert select_details["total_count"] == 3  # Found 3 times in strings

        # Check string location details
        string_locs = [loc for loc in select_details["locations"] if loc["type"] == "string"]
        assert len(string_locs) == 1
        assert string_locs[0]["count"] == 3
        assert set(string_locs[0]["lines"]) == {15, 18, 22}  # All 3 line numbers
