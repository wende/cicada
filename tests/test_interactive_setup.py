...
class TestShowFullInteractiveSetup:
    ...
    def test_non_elixir_project_exits(self, tmp_path, capsys):
        """Test that project with no language markers shows error and exits"""
        from cicada.interactive_setup import show_full_interactive_setup

        # No project marker files (no mix.exs, no pyproject.toml, etc.)
        with pytest.raises(SystemExit) as exc_info:
        ...
        assert "Expected Python markers" in captured.out or "Elixir marker" in captured.out
